...
public class LoRaTrackerMokoSupport extends MokoBleLib {
    private HashMap<OrderCHAR, BluetoothGattCharacteristic> mCharacteristicMap;

    private static volatile LoRaTrackerMokoSupport INSTANCE;

    private Context mContext;

    private LoRaTrackerMokoSupport() {
        //no instance
    }

    public static LoRaTrackerMokoSupport getInstance() {
        if (INSTANCE == null) {
            synchronized (LoRaTrackerMokoSupport.class) {
                if (INSTANCE == null) {
                    INSTANCE = new LoRaTrackerMokoSupport();
                }
            }
        }
        return INSTANCE;
