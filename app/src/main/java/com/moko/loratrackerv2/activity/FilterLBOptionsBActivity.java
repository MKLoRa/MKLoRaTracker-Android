...
public class FilterLBOptionsBActivity extends BaseActivity implements SeekBar.OnSeekBarChangeListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tvTitle.setText("Location Beacon Filter Option B");
        ...
        if (!LoRaTrackerMokoSupport.getInstance().isBluetoothOpen()) {
            ...
            orderTasks.add(OrderTaskAssembler.getLBFilterRssiB());
            orderTasks.add(OrderTaskAssembler.getLBFilterMacB());
            orderTasks.add(OrderTaskAssembler.getLBFilterNameB());
            orderTasks.add(OrderTaskAssembler.getLBFilterUUIDB());
            orderTasks.add(OrderTaskAssembler.getLBFilterMajorRangeB());
            orderTasks.add(OrderTaskAssembler.getLBFilterMinorRangeB());
            orderTasks.add(OrderTaskAssembler.getLBFilterAdvRawDataB());
                            ...
                            if (flag == 0x01) {
                                ...
                                switch (configKeyEnum) {
                                    ...
                                    case KEY_LOCATION_FILTER_ADV_RAW_DATA_B:
                                        ...
                                        if (savedParamsError) {
                                            ToastUtils.showToast(FilterLBOptionsBActivity.this, "Opps！Save failed. Please check the input characters and try again.");
                                        } else {
                                        ...
                                        if (length > 0) {
                                            ...
                                            if (length > 1) {
                                                ...
                                                for (int i = 0, l = rawDataBytes.length; i < l; ) {
                                                    View v = LayoutInflater.from(FilterLBOptionsBActivity.this).inflate(R.layout.loratracker_item_raw_data_filter, llRawDataFilter, false);
                ...
                if (BluetoothAdapter.ACTION_STATE_CHANGED.equals(action)) {
                    ...
                    switch (blueState) {
                        case BluetoothAdapter.STATE_TURNING_OFF:
                            dismissSyncProgressDialog();
                            FilterLBOptionsBActivity.this.setResult(RESULT_OK);
                            finish();
                            break;
                    }
                }
    ...
    private void saveParams() {
        ...
        orderTasks.add(OrderTaskAssembler.setLBFilterMinorRangeB(
        ...
        orderTasks.add(OrderTaskAssembler.setLBFilterAdvRawDataB(filterRawAdvDataEnable ? filterRawDatas : null
                , cbRawAdvData.isChecked()));
        orderTasks.add(OrderTaskAssembler.setLBFilterSwitchB(filterSwitchEnable ? 1 : 0));
